...
export async function webSearch(
    ...
    if (candidates.length === 0) return missingWebSearchConfigResult(ctx);
    ...
    for (let i = 0; i < candidates.length; i++) {
        ...
        if (isFallback) {
            onUpdate?.({
                content: [{
                    type: "text",
                    text: `Primary search model unavailable (${lastError?.message || "transient error"}). Retrying with fallback: ${describeModel(model)}...`
                }],
                details: { fallback: true, model: describeModel(model) }
            });
        }
        ...
        try {
            ...
            return formatResult(summary, {
                ...
                model: model.id,
                fallbackUsed: isFallback ? true : undefined,
    ...
    return errorResult(lastError || new Error("Web search failed"));
}
